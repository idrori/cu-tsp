""" 
  ...
  alpha carbon 3D coordinate to distance matrix prediction.
...
def train_model(model, fold_start, fold_end):
  ...
  # Preprocess data
  X_train, train_dcalphas = preprocess_data(training_data, 'dcalpha')
...
if __name__ == '__main__':
  ...
  try:
    ...
    with open(os.path.join(COORDS_DCALPHA_PATH, 'rnn_coords_dcalpha.json'), 'w') as fh:
      fh.write(rnn_json)
      recurrent_model.save_weights(os.path.join(COORDS_DCALPHA_PATH, 'weights_coords_dcalpha.h5'))
      print('Model saved.')
  except:
