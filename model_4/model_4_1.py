""" 
  ...
  network that predicts 3D coordinates and generates alpha-carbon
...
def get_model():
  ...
  return Model(inputs=[aa_seq, ss_seq, pssm], outputs=dcalpha)
...
def get_msa_model():
  ...
  return Model(inputs=[aa_seq, ss_seq, pssm, msa_onehot, msa_embed], outputs=dcalpha)
...
def get_no_q8_model():
  ...
  # PSSM
  pssm = Input(shape=(None,21))
  bigru_2 = Bidirectional(CuDNNGRU(units=100, return_sequences=True))(pssm)
  output_2 = Bidirectional(CuDNNGRU(units=64, return_sequences=True))(bigru_2)
  ...
  return Model(inputs=[aa_seq, pssm], outputs=dcalpha)
...
def train_model(model, fold_start, fold_end, msa_flag=False, no_q8_flag=False,
  ...
  training_data = load_data(fold_start, fold_end, msa_flag=msa_flag,
                            no_q8_flag=no_q8_flag, coords_flag=True)
  
  # Preprocess data
  X_train, train_dcalphas = preprocess_data(training_data, 'dcalpha', msa_flag=msa_flag)
  ...
  if ckpt_flag:
    try:
      ckpt_path = os.path.join(RESULTS_PATH, 'model_4_1_{}.h5'.format(fold_end))
