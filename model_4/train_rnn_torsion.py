""" 
  ...
  torsion (dihedral) angle prediction.
...
# Returns our proposed RNN encoder-decoder model for torsion angle prediction
def get_torsion_model():
  # AA Sequence
  ...
  final_1 = Bidirectional(CuDNNGRU(100, return_sequences=True))(concat)
  final_2 = Bidirectional(CuDNNGRU(64, return_sequences=True))(final_1)
  final_3 = Bidirectional(CuDNNGRU(32, return_sequences=True))(final_2)
  angles = TimeDistributed(Dense(2), name='torsion_output')(final_3)

  return Model(inputs=[aa_seq,ss_seq,pssm], outputs=angles)
...
def train_model(model, fold_start, fold_end):
  ...
  # Load data
  training_data = load_data(fold_start, fold_end)
  
  # Preprocess data
  X_train, train_angles = preprocess_data(training_data, 'torsion')
  
  # Train
  for i in tqdm(range(len(X_train))):
    x = [np.array(X_train[i][0])[np.newaxis,:], np.array(X_train[i][1])[np.newaxis,:], X_train[i][2][np.newaxis,:]]
    y = train_angles[i][np.newaxis,:]
    if (i % 50 == 0): 
...
if __name__ == '__main__':
  ...
  recurrent_model = get_torsion_model()
  ...
  try:
    ...
    with open(os.path.join(TORSION_PATH, 'rnn_torsion.json'), 'w') as fh:
      fh.write(rnn_json)
      recurrent_model.save_weights(os.path.join(TORSION_PATH, 'weights_torsion.h5'))
      print('Model saved.')
  except:
